...
APP_MAIN=com.wusiq.Application
...
start(){
    ...
    if [ $processStatus == 1 ]; then
        ...
        nohup $JAVA_HOME/bin/java -Djdk.tls.namedGroups="secp256k1" $JAVA_OPTS -Djava.library.path=$CONF_DIR -cp $CLASSPATH $APP_MAIN >> $LOG_DIR/application.out 2>&1 &
